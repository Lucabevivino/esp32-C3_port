...
{ 
    ...
    SYSTEM_PERIP_CLK_EN0_REG |= BIT(SYSTEM_SYSTIMER_CLK_EN_RST);
    SYSTEM_PERIP_RST_EN0_REG |= BIT(SYSTEM_SYSTIMER_CLK_EN_RST);
    SYSTEM_PERIP_RST_EN0_REG &= ~BIT(SYSTEM_SYSTIMER_CLK_EN_RST);
    
    // Configure UNIT0: Stall on CPU0 halt, enable counter
    SYSTIMER_CONF_REG &= ~BIT(SYSTIMER_TARGET0_WORK_EN);
    SYSTIMER_CONF_REG |= BIT(SYSTIMER_TIMER_UNIT0_WORK_EN) | BIT(SYSTIMER_TIMER_UNIT0_CORE0_STALL_EN);
    ...
    SYSTIMER_TARGET0_CONF_REG &= ~BIT(SYSTIMER_UNIT0_SEL);
    ...
    SYSTIMER_TARGET0_CONF_REG |= BIT(SYSTIMER_TARGET0_PERIOD_MODE);   
    SYSTIMER_CONF_REG |= BIT(SYSTIMER_TARGET0_WORK_EN);  // Start comparing
    ...
    INTERRUPT_CORE0_CPU_INT_ENABLE_REG |= BIT(SYSTIMER_CPU_LINE_MASK);
